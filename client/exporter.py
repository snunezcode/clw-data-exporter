...
class classCloudwatchExporter():
    ...
    def get_pi_metrics(self, resource_id, service_type, query, start_date, end_date, period):
        result = self.pi_client.get_resource_metrics(
        ...
        self.file.write_json({ "file_name" : self.output_path + self.get_uid(23) + ".pi", "open_mode" : "w", "content" : json.dumps(result, indent=2, default=self.default) })
    ...
    def get_metrics(self, template, properties ):
        ...
        for metric in metric_list['metrics']:
            ...
            if (result['is_null']==False):
                self.file.write_json({ "file_name" : self.output_path + self.get_uid(23) + ".mt", "open_mode" : "w", "content" : result })
...
class classRDS():
    ...
    def gather_metadata(self,instance,uid):
    ...
    def export_metrics(self):
        ...
        for instance in self.instances:
            metadata.append(
                                ...
                                "resourceDefinition" : self.gather_metadata(instance['name'],instance['uid']) 
                                }
            )
        self.file.write_all({ "file_name" : self.output_path + "metadata.json", "open_mode" : "w", "content" : json.dumps({ "exportParameters" : export_parameters, "metadata" : metadata}, indent=4, default=self.default) })
        ...
        table_template = self.file.read_all({"file_name" : 'templates/rds.json' })
        for instance in self.instances:
            print("Processing Object : Instance : ",instance['name'] )
            ...
            properties = [
                            { "Name" : "ObjectType", "Value" : "RDSInstance" }, 
            ...
            self.clw.get_pi_metrics("db-IOZIWKJAZOLRUJIUKBPWJDCGEI", "RDS", query, start_date, end_date, int(self.period))
            '''
...
#from dpa360 import classDynamoDB, classImporter
